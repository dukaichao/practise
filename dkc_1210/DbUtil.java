...
public class DbUtil {
	public static void main(String[] args) {
		...
		String url = "jdbc:mysql://123.0.0.1:8888/du";
		try {
			Class.forName("jdbc.mysql.dkc.Driver");
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		try {
			con = DriverManager.getConnection(url, "dkc", "112");
			stat = con.createStatement();
			String name = "dz";
			String pwd = "223";
			String insert = "insert into userfo(user,pwd) values(name,pwd)";
			stat.executeQuery(insert);
			rst = stat.executeQuery("selet * from userfo");
			while(rst.next()) {
				String n = rst.getString("n");
				String p = rst.getString("p");
